    ...
    <div class="container">
        @if($session)
            <div class="alert alert-danger" id="myDiv">
                Fout bij het aanmaken van een les
        ...
        <form class="row" action="{{ route('lessen.store')}}" method="POST">
            @csrf
            <div class="col-12">
                <div class="form-group">
                    <label for="lesdoel">lesdoel</label>
                    <input type="text" class="form-control" id="lesdoel" name="lesdoel" placeholder="lesdoel" required
                           @if($session)
                           @foreach($session['error'] as $key => $value)
                           @if($key == 'lesdoel')
                           value="{{$value}}"
